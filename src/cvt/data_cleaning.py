...
from file_paths import (FLOODING_RAW_IN, FLOODING_MODEL_IN)
...
def clean_infrastructure(boundary):
    ...
    tfn_rail_links = get_rail_links(boundary, cfg.tfn_rail_links)
...
def clean_roads(boundary):
    ...
    clean_os_roads(boundary)
    clean_noham_roads(boundary)

def clean_os_roads(boundary):
    """Reads and cleans OS Open Roads dataset, then writes to file"""
    os_road = gpd.read_file(cfg.os_road)
    ...
    write_to_file(tfn_os_road, cfg.data.model_input / "Infrastructure" / "Road" / "TfN OS Road" / "tfn_os_road.shp")

def clean_noham_roads(boundary):
    """Reads and cleans 2023 and 2048 NoHAM network datasets, then writes to file"""
    noham = {
        '2023': gpd.read_file(cfg.noham_2023),
        '2048': gpd.read_file(cfg.noham_2048),
    }
    ...
    for year, noham_network in noham.items():
        ...
        write_to_file(tfn_noham_network,
                      cfg.model_input / "Infrastructure" / "Road" / f"TfN NoHAM {year}" / f"tfn_noham_{year}.shp")
...
def clean_other(boundary, rail_links):
    ...
    clean_bus_stops(boundary)
    clean_petrol_stations(boundary)
    clean_charging_sites(boundary)
    clean_ncn(boundary)

    os_mm_net_node = read_os_mm_node_network(cfg.os_mmrn)
...
def clean_bus_stops(boundary):
    """Reads, combines and cleans regional bus stops datasets, then writes to file"""
    bus_stops_ne = pd.read_csv(cfg.bus_stops_ne)  # North East
    bus_stops_nw = pd.read_csv(cfg.bus_stops_nw)  # North West
    bus_stops_ys = pd.read_csv(cfg.bus_stops_ys)  # Yorkshire
    ...
    write_to_file(tfn_bus_stops, cfg.model_input / "Infrastructure" / "Other" / "TfN Bus Stops" / "tfn_bus_stops.shp")

def clean_petrol_stations(boundary):
    """Reads and cleans POI data, filters for petrol stations, and writes to file"""
    poi_uk = gpd.read_file(f"zip://{cfg.poi_uk.zip_path}!{cfg.poi_uk.internal_path}")
    ...
    write_to_file(tfn_petrol,
                  cfg.model_input / "Infrastructure" / "Other" / "TfN Petrol Stations" / "tfn_petrol_stations.shp")
...
def clean_train_stations(os_mm_net_node, boundary):
    ...
    write_to_file(tfn_train_stations,
                  cfg.model_input / "Infrastructure" / "Other" / "TfN OS Train Stations" / "tfn_train_stations.shp")
...
def clean_tram_stations(os_mm_net_node, boundary):
    ...
    write_to_file(tfn_tram_stations,
                  cfg.model_input / "Infrastructure" / "Other" / "TfN OS Tram Stations" / "tfn_tram_stations.shp")
...
def clean_rapid_transport_stations(os_mm_net_node, boundary):
    ...
    write_to_file(tfn_rapid_transport_stations,
                  cfg.model_input / "Infrastructure" / "Other" /
...
def clean_ferry_terminals(os_mm_net_node, boundary):
    ...
    write_to_file(tfn_ferry_terminals,
                  cfg.model_input / "Infrastructure" / "Other" / "TfN OS Ferry Terminals" / "tfn_ferry_terminals.shp")
...
def clean_tram_network(tfn_rail_links):
    ...
    write_to_file(tfn_tram_links,
                  cfg.model_input / "Infrastructure" / "Other" / "TfN OS Tram Network" / "tfn_os_tram_links.shp")
...
def clean_charging_sites(boundary):
    """Reads and cleans ZapMap charging sites data, then writes to file"""
    chg_sites = pd.read_csv(cfg.zapmap)
    ...
    write_to_file(tfn_chg_sites,
                  cfg.model_input / "Infrastructure" / "Other" / "TfN EV Charging Sites" / "tfn_chg_sites.shp")

def clean_ncn(boundary):
    """Reads and cleans National Cycle Network data, then writes to file"""
    ncn = gpd.read_file(cfg.ncn_sustrans)
    ...
    write_to_file(tfn_ncn, cfg.model_input / "Infrastructure" / "Other" / "TfN NCN" / "tfn_ncn.shp")
...
def clean_extreme_weather(boundary):
    ...
    tfn_common_grid = clean_common_grid(boundary)

    clean_temp_max(tfn_common_grid)
    clean_temp_min(tfn_common_grid)
    clean_summer_precip(tfn_common_grid)
    clean_winter_precip(tfn_common_grid)
    clean_rain_days(boundary)
    clean_drought_index(boundary)
    clean_hot_summer_days(tfn_common_grid)
    clean_extreme_summer_days(tfn_common_grid)
    clean_frost_days(tfn_common_grid)
    clean_icing_days(tfn_common_grid)
    clean_wind_speed(boundary)
    clean_wind_driven_rain(boundary)

def clean_common_grid(boundary):
    """Creates and prepares common grid DataFrame for variables on same 12km British National Grid"""
    temp_max = gpd.read_file(f"zip://{cfg.max_temp_summer.zip_path}!{cfg.max_temp_summer.internal_path}")
    ...
    write_to_file(tfn_common_grid, cfg.model_input / "Other" / "TfN Common Grid" / "tfn_common_grid.shp")
    return tfn_common_grid

def clean_temp_max(grid):
...
def clean_temp_min(grid):
    """Reads and cleans min winter temperature change projections, then writes to file"""
    temp_min = gpd.read_file(f"zip://{cfg.min_temp_winter.zip_path}!{cfg.min_temp_winter.internal_path}")
...
def clean_summer_precip(grid):
    """Reads and cleans summer precipitation change projections, then writes to file"""
    precip_sum = gpd.read_file(f"zip://{cfg.precip_summer.zip_path}!{cfg.precip_summer.internal_path}")
...
def clean_winter_precip(grid):
    """Reads and cleans winter precipitation change projections, then writes to file"""
    precip_win = gpd.read_file(f"zip://{cfg.precip_winter.zip_path}!{cfg.precip_winter.internal_path}")
...
def clean_rain_days(boundary):
    """Reads and cleans 10mm rain days observations, then writes to file"""
    rain_days = gpd.read_file(f"zip://{cfg.rain_days.zip_path}!{cfg.rain_days.internal_path}")
    ...
    write_to_file(tfn_rain_days, cfg.model_input / "Hazards" / "Extreme Weather" /
                  "TfN 10mm Rain Days 1991-2020" / "tfn_rain_days.shp")

def clean_drought_index(boundary):
    """Reads and cleans drought severity index data, then writes to file"""
    drought_index = gpd.read_file(f"zip://{cfg.drought_index.zip_path}!{cfg.drought_index.internal_path}")
    ...
    write_to_file(tfn_drought, cfg.model_input / "Hazards" / "Extreme Weather" /
                  "TfN Drought Severity Index" / "tfn_drought_index.shp")

def clean_hot_summer_days(grid):
    """Reads and cleans hot summer days projections, then writes to file"""
    hot_days = gpd.read_file(f"zip://{cfg.hot_days.zip_path}!{cfg.hot_days.internal_path}")
    ...
    write_to_file(tfn_hot_days, cfg.model_input / "Hazards" / "Extreme Weather" /
                  "TfN Hot Summer Days Projections" / "tfn_hot_days.csv",
                  csv=True)

def clean_extreme_summer_days(grid):
    """Reads and cleans extreme summer days projections, then writes to file"""
    extr_days = gpd.read_file(f"zip://{cfg.extreme_summer_days.zip_path}!{cfg.extreme_summer_days.internal_path}")
...
def clean_frost_days(grid):
    """Reads and cleans frost days projections, then writes to file"""
    frost_days = gpd.read_file(f"zip://{cfg.frost_days.zip_path}!{cfg.frost_days.internal_path}")
    ...
    write_to_file(tfn_frost_days, cfg.model_input / "Hazards" / "Extreme Weather" /
                  "TfN Frost Days Projections" / "tfn_frost_days.csv",
                  csv=True)

def clean_icing_days(grid):
    """Reads and cleans icing days projections, then writes to file"""
    ice_days = gpd.read_file(f"zip://{cfg.icing_days.zip_path}!{cfg.icing_days.internal_path}")
    ...
    write_to_file(tfn_ice_days, cfg.model_input / "Hazards" / "Extreme Weather" /
                  "TfN Icing Days Projections" / "tfn_ice_days.csv", csv=True)

def clean_wind_speed(boundary):
    """Reads wind speed projections, calculates exceedance and percentiles, cleans, then writes to file"""
    windspd_c_combined = read_wind_speed_reduce(cfg.wind_spd_current, 'c')
    windspd_f_combined = read_wind_speed_reduce(cfg.wind_spd_forecast, 'f')
    ...
    write_to_file(tfn_windspd, cfg.model_input / "Hazards" / "Extreme Weather" /
...
def clean_wind_driven_rain(boundary):
    """Reads and cleans wind driven rain index data, then writes to file"""
    wdr = gpd.read_file(f"zip://{cfg.wdr_index.zip_path}!{cfg.wdr_index.internal_path}")
    ...
    write_to_file(tfn_wdr, cfg.model_input / "Hazards" / "Extreme Weather" /
...
def clean_ground_stability(boundary):
    ...
    clean_geosure(boundary)
    clean_geoclimate(boundary)

def clean_geosure(boundary):
    """Cleans GeoSureHexGrids data, merges by nearest centroids, then writes to file"""
    geosure_layers = {
        'cd': gpd.read_file(f"zip://{cfg.geosure.zip_path}!/" + cfg.geosure.collapsible_deposits),
        'cg': gpd.read_file(f"zip://{cfg.geosure.zip_path}!/" + cfg.geosure.compressible_ground),
        'ls': gpd.read_file(f"zip://{cfg.geosure.zip_path}!/" + cfg.geosure.landslides),
        'rs': gpd.read_file(f"zip://{cfg.geosure.zip_path}!/" + cfg.geosure.running_sand),
        'ss': gpd.read_file(f"zip://{cfg.geosure.zip_path}!/" + cfg.geosure.shrink_swell),
        'sr': gpd.read_file(f"zip://{cfg.geosure.zip_path}!/" + cfg.geosure.soluble_rocks)
    }
    ...
    write_to_file(tfn_geosure, cfg.model_input / "Hazards" / "Ground Stability" / "TfN GeoSure" / "tfn_geosure.shp")

def clean_geoclimate(boundary):
    """Reads and cleans GeoClimate Shrink-Swell data, then writes to file"""
    for year in ['2030', '2070']:
        gdf = gpd.read_file(cfg.geo_shrink_swell[year])
        gdf.rename(columns={'CLASS': 'ss_geo_risk'}, inplace=True)
        gdf = gdf[['ss_geo_risk', 'geometry']]
        tfn_gdf = clip_to_boundary(gdf, boundary)
        tfn_gdf = explode_to_polygons(tfn_gdf)
        write_to_file(tfn_gdf, cfg.model_input / "Hazards" / "Ground Stability" /
...
def clean_coastal_erosion(boundary):
    ...
    clean_giz(boundary)
    clean_ncerm(boundary)

def clean_giz(boundary):
    """Cleans Ground Instability Zones data from NCERM, then writes to file"""
    ncerm_giz = gpd.read_file(f"zip://{cfg.ce_zip_path}!{cfg.giz}")
...
def clean_freight_demand(boundary):
    ...
    tfn_freight_network_demand = read_freight_demand(cfg.freight_demand, boundary)
...
def clean_noham_flows():
    ...
    link_flows = aggregate_link_flows_year(cfg.noham_demand.zip_path, cfg.noham_demand.output_path)

    for year, tp in {'2023': 'c', '2048': 'f'}.items():
        tfn_noham_flows = link_flows[year]
        tfn_noham_net_flows = merge_noham_flow_network(
            tfn_noham_flows,
            cfg.model_in / "Infrastructure" / "Road" / f"TfN NoHAM {year}" / f"tfn_noham_{year}.shp"
        )
        write_to_file(tfn_noham_net_flows,
                      cfg.model_in / "Impact" / "TfN NoHAM Flows" / year / f"tfn_noham_net_flows_{tp}.gpkg",
                      driver="GPKG")
