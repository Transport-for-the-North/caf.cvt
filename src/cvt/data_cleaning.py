...
def df_to_gdf(df, x_col, y_col, crs):
    ...
    geometry = [Point(xy) for xy in zip(bus_stops[x_col], bus_stops[y_col])]  # Create geometry from lat/lon
...
## TfN Boundary
tfn_boundary = gpd.read_file(OTHER_IN / "TfN_Boundary" / "Transport_for_the_north_boundary_2020_generalised.shp")
...
#### OS Open Roads
os_road = gpd.read_file(ROAD_IN / "TfN OS Open Roads" / "os_open_gb_road_links_tfn.shp")
...
os_road[['road_number', 'name', 'function']] = os_road[['road_number', 'name', 'function']].replace(0, pd.NA) # Replace 0s with NA
...
tfn_os_road = clip_to_boundary(os_road, tfn_boundary) # Clip to spatial boundary
tfn_os_road.to_file(ROAD_OUT / "TfN OS Road" / "tfn_os_road.shp") # Write to file

#### NoHAM Road Network
noham_2023 = gpd.read_file(ROAD_IN / "NoHAM 2023" / "NoHAM_Decarb_DM_Core_2023_carbon.shp")
noham_2048 = gpd.read_file(ROAD_IN / "NoHAM 2048" / "NoHAM_Decarb_DM_Core_2048_carbon.shp")

noham = {'2023': noham_2023, '2048': noham_2048}
tfn_noham = {'2023': pd.NA, '2048': pd.NA}
for year, noham_network in noham.items():
    ...
    tfn_noham_network = clip_to_boundary(noham_network, tfn_noham)
    tfn_noham_network.to_file(ROAD_OUT / f"TfN NoHAM {year}" / f"tfn_noham_{year}.shp")

### RAIL

#### Rail Network
tfn_rail_links = gpd.read_file(RAIL_IN / "OS Rail Network" / "TfN_Area_tfn_ntwk_railwaylink.gpkg")
...
tfn_rail_links = tfn_rail_links[~tfn_rail_links['description'].isin(['Preserved', 'Funicular', 'Mineral', 'Static Museum'])]
...
tfn_rail_links[['description', 'structure', 'physicallevel', 'railwayuse', 'trackrepresentation']] = (
    tfn_rail_links[['description', 'structure', 'physicallevel', 'railwayuse', 'trackrepresentation']].replace(0, 'N/A'))
...
#### Bus Stops
bus_stops_ne = pd.read_csv(OTHER_IN / "Bus Stops" / "bus_stops_ne.csv") # North East
bus_stops_nw = pd.read_csv(OTHER_IN / "Bus Stops" / "bus_stops_nw.csv") # North West
bus_stops_ys = pd.read_csv(OTHER_IN/ "Bus Stops" / "bus_stops_ys.csv") # Yorkshire
...
tfn_bus_stops = clip_to_boundary(bus_stops, tfn_boundary) # Clip to TfN boundary
tfn_bus_stops.to_file(OTHER_OUT / "TfN Bus Stops" / "tfn_bus_stops.shp")

#### Petrol Stations
poi_uk = gpd.read_file(f"zip://{OTHER_IN / "poi_uk.zip"}!poi_uk.gpkg")
...
tfn_petrol = clip_to_boundary(petrol_stations, tfn_boundary)
tfn_petrol.to_file(OTHER_OUT / "TfN Petrol Stations" / "tfn_petrol_stations.shp")

#### OS Multi-Modal Routing Network Nodes
os_mm_net_node = gpd.read_file(OTHER_IN / "OS Multi-Modal Routing Network" / "OSMulti-modalRoutingNetwork.gpkg",
...
tfn_train_stations = clip_to_boundary(train_stations, tfn_boundary)
...
tfn_tram_stations = clip_to_boundary(tram_stations, tfn_boundary)
tfn_tram_stations.to_file(OTHER_OUT / "TfN OS Tram Stations" / "tfn_tram_stations.shp")

#### Metro Stations
metro_stations = os_mm_net_node[os_mm_net_node['os_nodetype'].str.contains('Underground System', case=False, na=False)]
metro_stations.drop(columns=['os_nodetype'], inplace=True)
tfn_metro_stations = clip_to_boundary(metro_stations, tfn_boundary)
...
tfn_ferry_stations = clip_to_boundary(ferry_stations, tfn_boundary)
...
tfn_bus_coach_stations = clip_to_boundary(bus_coach_stations, tfn_boundary)
...
#### Charging Points
chg_pts = pd.read_csv(OTHER_IN / "Zap Map Data - full data set Oct 25.csv")
...
tfn_chg_pts = clip_to_boundary(chg_pts_gdf, tfn_boundary)
tfn_chg_pts.to_file(OTHER_OUT / "TfN Charging Points" / "tfn_chg_pts.shp")

#### National Cycle Network
ncn = gpd.read_file(OTHER_IN / "NCN Sustrans" / "National_Cycle_Network_Public.shp")
...
tfn_ncn = clip_to_boundary(ncn, tfn_boundary)
...
#### Summer Max Temperature Change Projections
temp_max = gpd.read_file(
    f"zip://{EXTREME_WEATHER_IN / "Summer_Maximum_Temperature_Change___Projections_12km_grid.zip"}!summer_maximum_temperature_change_projections_12km.shp")
...
tfn_temp_max = clip_to_boundary(temp_max, tfn_boundary)
...
#### Winter Min Temperature Change Projections
temp_min = gpd.read_file(
    f"zip://{EXTREME_WEATHER_IN / "Winter_Minimum_Temperature_Change___Projections_12km_grid.zip"}!winter_minimum_temperature_change_projections_12km.shp")
...
tfn_temp_min = clip_to_boundary(temp_min, tfn_boundary)
