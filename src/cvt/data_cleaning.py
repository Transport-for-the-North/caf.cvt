...
_BNG_CRS = os.getenv("_BNG_CRS", "EPSG:27700")
...
def _nearest_centroids(gdf1: gpd.GeoDataFrame, gdf2: gpd.GeoDataFrame) -> gpd.GeoDataFrame:
    ...
    gdf1 = gdf1.to_crs(_BNG_CRS)
    gdf2 = gdf2.to_crs(_BNG_CRS)
...
def _clean_noham_roads(config: model_config.Config, boundary: gpd.GeoDataFrame) -> None:
    ...
    for scenario, noham_entry in config.infrastructure.road.noham.items():
        ...
        LOG.info(
            "NoHAM network %s filtered - %s of %s (%s percent) rows removed",
            year,
            filter_removed,
            len_before_filter,
            (filter_removed / len_before_filter) * 100,
...
def _clean_hazard_grid(
    ...
    LOG.info(
        ...
        filter_removed, len_before_filter,
        (filter_removed / len_before_filter) * 100)
    write_to_file(
        tfn_hazard_grid,
...
def _clean_temp_max(config: model_config.Config, grid: gpd.GeoDataFrame) -> None:
    ...
    LOG.info(
        "Summer max temperature change projections filtered - %s of %s (%s percent) rows "
        "removed",
        filter_removed,
        len_before_filter,
        (filter_removed / len_before_filter) * 100,
...
def _clean_temp_min(config: model_config.Config, grid: gpd.GeoDataFrame) -> None:
    ...
    LOG.info(
        "Winter minimum temperature change projections filtered - %s of %s (%s percent) "
        "rows removed",
        filter_removed,
        len_before_filter,
        (filter_removed / len_before_filter) * 100,
...
def _clean_summer_precip(config: model_config.Config, grid: gpd.GeoDataFrame) -> None:
    ...
    LOG.info(
        "Summer precipitation change projections filtered - %s of %s (%s percent) rows "
        "removed",
        filter_removed,
        len_before_filter,
        (filter_removed / len_before_filter) * 100,
...
def _clean_winter_precip(config: model_config.Config, grid: gpd.GeoDataFrame) -> None:
    ...
    LOG.info(
        "Winter precipitation change projections filtered - %s of %s (%s percent) rows "
        "removed",
        filter_removed,
        len_before_filter,
        (filter_removed / len_before_filter) * 100,
...
def _clean_geosure(config: model_config.Config, boundary: gpd.GeoDataFrame) -> None:
    ...
    for code, geosure_data in geosure_layers.items():
        ...
        LOG.info(
            "GeoSure %s filtered - %s of %s (%s percent) rows removed",
...
def _clean_geoclimate(config: model_config.Config, boundary: gpd.GeoDataFrame) -> None:
    ...
    for year, filepath in config.hazards.ground_stability.geo_shrink_swell:
        geoclimate_data = gpd.read_file(filepath)
        geoclimate_data = geoclimate_data.rename(
            columns={"CLASS": "shrink_swell_geoclimate_risk"})
        geoclimate_data = geoclimate_data[["shrink_swell_geoclimate_risk", "geometry"]]
        ...
        LOG.info(
            "GeoClimate %s filtered - %s of %s (%s percent) rows removed",
            year,
            filter_removed,
            len_before_filter,
            (filter_removed / len_before_filter) * 100)
...
def _clean_ncerm(config: model_config.Config, boundary: gpd.GeoDataFrame) -> None:
    ...
    for year in ["2055", "2105"]:
        ...
        erosion_data = erosion_data[["smp_name", "geometry"]]
        ...
        LOG.info(
            "NCERM %s filtered - %s of %s (%s percent) rows removed",
            year,
            filter_removed,
            len_before_filter,
            (filter_removed / len_before_filter) * 100)
