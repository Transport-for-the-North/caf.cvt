...
PASSENGER_RAIL_MODEL_INPUT_PATH = pathlib.Path(
    "Infrastructure/Rail/TfN OS Passenger Rail/tfn_pass_rail_links.gpkg"
)

FREIGHT_RAIL_MODEL_INPUT_PATH = pathlib.Path(
    "Infrastructure/Rail/TfN OS Freight Rail/tfn_freight_rail_links.gpkg"
)
...
BUS_STOPS_MODEL_INPUT_PATH = pathlib.Path(
    "Infrastructure/Other/TfN Bus Stops/tfn_bus_stops.gpkg"
)

PETROL_STATIONS_MODEL_INPUT_PATH = pathlib.Path(
    "Infrastructure/Other/TfN Petrol Stations/tfn_petrol_stations.gpkg"
)


TRAIN_STATIONS_MODEL_INPUT_PATH = pathlib.Path(
    "Infrastructure/Other/TfN OS Train Stations/tfn_train_stations.gpkg"
)
TRAM_STATIONS_MODEL_INPUT_PATH = pathlib.Path(
    "Infrastructure/Other/TfN OS Tram Stations/tfn_tram_stations.gpkg"
)
RAPID_TRANSPORT_STATIONS_MODEL_INPUT_PATH = pathlib.Path(
    "Infrastructure/Other/TfN OS Rapid Transport Stations/tfn_rapid_transport_stations.gpkg"
)
FERRY_TERMINALS_MODEL_INPUT_PATH = pathlib.Path(
    "Infrastructure/Other/TfN OS Ferry Terminals/tfn_ferry_terminals.gpkg"
)
BUS_COACH_STATIONS_MODEL_INPUT_PATH = pathlib.Path(
    "Infrastructure/Other/TfN OS Bus Coach Stations/tfn_bus_coach_stations.gpkg"
)
TRAM_NETWORK_MODEL_INPUT_PATH = pathlib.Path(
    "Infrastructure/Other/TfN OS Tram Network/tfn_os_tram_links.gpkg"
)
...
HAZARD_GRID_MODEL_INPUT_PATH = pathlib.Path(
    "Hazards/Extreme Weather/TfN Hazard Grid/tfn_hazard_grid.gpkg"
)

TEMP_MAX_MODEL_INPUT_PATH = pathlib.Path(
    "Hazards/Extreme Weather/TfN Summer Max Temperature Change Projections/tfn_temp_max.csv"
)
TEMP_MIN_MODEL_INPUT_PATH = pathlib.Path(
    "Hazards/Extreme Weather/TfN Winter Min Temperature Change Projections/tfn_temp_min.csv"
)

SUMMER_PRECIP_MODEL_INPUT_PATH = pathlib.Path(
    "Hazards/Extreme Weather/TfN Summer Precipitation Change Projections/tfn_precip_sum.csv"
)
WINTER_PRECIP_MODEL_INPUT_PATH = pathlib.Path(
    "Hazards/Extreme Weather/TfN Winter Precipitation Change Projections/tfn_precip_win.csv"
)

RAIN_DAYS_MODEL_INPUT_PATH = pathlib.Path(
    "Hazards/Extreme Weather/TfN 10mm Rain Days 1991-2020/tfn_rain_days.gpkg"
)
DROUGHT_INDEX_MODEL_INPUT_PATH = pathlib.Path(
    "Hazards/Extreme Weather/TfN Drought Severity Index/tfn_drought_index.gpkg"
)
HOT_SUMMER_DAYS_MODEL_INPUT_PATH = pathlib.Path(
    "Hazards/Extreme Weather/TfN Hot Summer Days Projections/tfn_hot_days.csv"
)
EXTREME_SUMMER_DAYS_MODEL_INPUT_PATH = pathlib.Path(
    "Hazards/Extreme Weather/TfN Extreme Summer Days Projections/tfn_extr_days.csv"
)
FROST_DAYS_MODEL_INPUT_PATH = pathlib.Path(
    "Hazards/Extreme Weather/TfN Frost Days Projections/tfn_frost_days.csv"
)
ICING_DAYS_MODEL_INPUT_PATH = pathlib.Path(
    "Hazards/Extreme Weather/TfN Icing Days Projections/tfn_ice_days.csv"
)
WIND_SPEED_MODEL_INPUT_PATH = pathlib.Path(
    "Hazards/Extreme Weather/TfN Wind Speed Projections/tfn_windspd.gpkg"
)

WIND_DRIVEN_RAIN_MODEL_INPUT_PATH = pathlib.Path(
    "Hazards/Extreme Weather/TfN Wind Driven Rain Index/tfn_wdr.gpkg"
)
...
FLOOD_SURFACE_WATER_MODEL_INPUT_PATH = pathlib.Path(
    "Hazards/Flooding/TfN RoFSW/tfn_rofsw.gpkg"
)
GEOSURE_MODEL_INPUT_PATH = pathlib.Path(
    "Hazards/Ground Stability/TfN GeoSure/tfn_geosure.gpkg"
)
GEOCLIMATE_SHRINK_SWELL_MODEL_INPUT_PATH = pathlib.Path(
    "Hazards/Ground Stability/BGS Shrink Swell/tfn_bgs_shrink_swell.gpkg"
)
GROUND_INSTABILITY_ZONES_MODEL_INPUT_PATH = pathlib.Path(
    "Hazards/Coastal Erosion/NCERM/tfn_ncerm_giz.gpkg"
)
...
FREIGHT_DEMAND_MODEL_INPUT_PATH = pathlib.Path(
    "Impact/TfN Freight Flows/tfn_freight_network_demand.gpkg"
)
...
GROUND_STABILITY_MODEL_INTERIM_OUTPUT_PATH = pathlib.Path(
    "TfN Ground Stability Risk/tfn_ground_stability_risk.gpkg"
)

COASTAL_EROSION_MODEL_INTERIM_OUTPUT_PATH = pathlib.Path(
    "TfN Coastal Erosion Risk/tfn_coastal_erosion_risk.gpkg"
)
